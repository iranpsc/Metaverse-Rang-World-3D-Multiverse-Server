...
const DEFAULT_TIMEOUT_MS = Number(process.env.GRPC_REALTIME_MAIN_SERVER_TWO_CLIENT_STATE_TIMEOUT_MS || 9000);
const SHOULD_KEEP_DATA = process.env.GRPC_REALTIME_MAIN_SERVER_TWO_CLIENT_STATE_KEEP_DATA === "1";
const EXPECTED_POSITION_X = 12.5;

//* این آبجکت لاگ‌های ساده تست وضعیت پلیر روی سرور اصلی را چاپ می‌کند.
const logger = {
    info: (message, data = {}) => console.log("[G8-MainServerTwoClientPlayerStateSmoke][INFO]", message, data),
    warn: (message, data = {}) => console.warn("[G8-MainServerTwoClientPlayerStateSmoke][WARN]", message, data),
    error: (message, data = {}) => console.error("[G8-MainServerTwoClientPlayerStateSmoke][ERROR]", message, data)
};
...
function createMainServerRegisterRequest(prefix) {
    ...
    return {
        email: `g8-main-state-${prefix}-${id}@test.local`,
        password: `Test_${id}_12345`,
        userName: `g8_state_${prefix}_${id}`.slice(0, 32)
    };
}
...
function createLobbyCreateRoomEnvelope(roomName) {
    return makeEnvelope({
        ...
        payload: {
            roomName,
            description: "main server grpc realtime two client player state smoke",
            visibility: "public",
            maxPlayers: 20,
            metadata: {
                source: "main_server_grpc_realtime_two_client_player_state_smoke"
            }
        },
        prefix: "create_room"
    });
...
function createPresencePlayerStateEnvelope(roomId, secondRegisterResponse) {
    return makeEnvelope({
        channel: "presence",
        type: "player_state",
        room: roomId,
        payload: {
            userId: secondRegisterResponse.user.id,
            userName: secondRegisterResponse.user.userName,
            position: {
                x: EXPECTED_POSITION_X,
                y: 1.25,
                z: -7.5
            },
            rotation: {
...
function assertCreateRoomAckFrame(createRoomEnvelope, frame) {
    ...
    const roomId = String(envelope.payload?.details?.room?.roomId ?? envelope.payload?.details?.roomId ?? "").trim();
...
//* این تابع مقدار ایکس موقعیت را از چند ساختار احتمالی پیام می‌خواند.
function readPositionX(envelope) {
    return Number(
        envelope.payload?.position?.x ??
        envelope.payload?.state?.position?.x ??
        envelope.payload?.x ??
        Number.NaN
    );
}

//* این تابع بررسی می‌کند پیام وضعیت پلیر کاربر دوم به کاربر اول رسیده باشد.
function assertPlayerStateFrame(frame, roomId, secondRegisterResponse) {
    const envelope = frame.envelope;

    if (envelope.ch !== "presence") throw new Error(`Expected presence channel but got ${envelope.ch}`);
    if (envelope.t !== "player_state") throw new Error(`Expected player_state type but got ${envelope.t}`);
    if (envelope.room !== roomId) throw new Error("Player state room id does not match created room");

    const userId = envelope.payload?.userId ?? envelope.payload?.state?.userId;

    if (userId !== secondRegisterResponse.user.id) {
        throw new Error("Player state user id does not match second user");
    }

    const positionX = readPositionX(envelope);

    if (positionX !== EXPECTED_POSITION_X) {
        throw new Error(`Player state position x mismatch: ${positionX}`);
    }
...
//* این تابع بررسی می‌کند اَک وضعیت پلیر برای کاربر دوم برگشته باشد.
function assertPlayerStateAckFrame(playerStateEnvelope, frame) {
    const envelope = frame.envelope;

    if (envelope.ch !== "system") throw new Error(`Expected system channel but got ${envelope.ch}`);
    if (envelope.t !== "ack") throw new Error(`Expected ack type but got ${envelope.t}`);
    if (envelope.replyTo !== playerStateEnvelope.id) throw new Error("Player state ack replyTo does not match message id");
    if (envelope.payload?.status !== "processed") throw new Error("Player state ack status is not processed");

    return true;
}

//* این تابع داده‌های ساخته‌شده تست را از دیتابیس پاک می‌کند.
async function cleanupMainServerTwoClientPlayerStateData({ ownerUserId = "", secondUserId = "" } = {}) {
...
//* این تابع کل تست دو کلاینت و وضعیت پلیر را روی سرور اصلی اجرا می‌کند.
async function runGrpcStreamingRealtimeMainServerTwoClientPlayerStateSmoke() {
    let ownerUserId = "";
    let secondUserId = "";
    ...
    try {
        ...
        const roomName = `g8 main state ${randomUUID().slice(0, 8)}`;
        ...
        const playerStateEnvelope = createPresencePlayerStateEnvelope(roomId, secondRegisterResponse);

        const ownerPlayerStatePromise = ownerStream.waitFor(
            (envelope) => envelope.ch === "presence" && envelope.t === "player_state" && envelope.payload?.userId === secondUserId,
            "owner receives second user player state"
        );

        const secondPlayerStateAckPromise = secondStream.waitFor(
            (envelope) => envelope.ch === "system" && envelope.t === "ack" && envelope.replyTo === playerStateEnvelope.id,
            "second receives player state ack"
        );

        secondStream.send(playerStateEnvelope);

        const [ownerPlayerStateFrame, secondPlayerStateAckFrame] = await Promise.all([
            ownerPlayerStatePromise,
            secondPlayerStateAckPromise
        ]);

        const playerStateResult = assertPlayerStateFrame(ownerPlayerStateFrame, roomId, secondRegisterResponse);
        assertPlayerStateAckFrame(playerStateEnvelope, secondPlayerStateAckFrame);

        logger.info("Grpc streaming realtime main server two client player state smoke passed", {
            target: DEFAULT_TARGET,
            ownerUserId,
            secondUserId,
            roomId,
            roomName,
            receivedType: ownerPlayerStateFrame.envelope.t,
            positionX: playerStateResult.positionX
        });
        ...
        const cleanup = await cleanupMainServerTwoClientPlayerStateData({
            ownerUserId,
            secondUserId
        });

        logger.info("Grpc streaming realtime main server two client player state cleanup completed", cleanup);
    }
}

//* این تابع خط فرمان تست دو کلاینت و وضعیت پلیر روی سرور اصلی را اجرا می‌کند.
async function main() {
    try {
        await runGrpcStreamingRealtimeMainServerTwoClientPlayerStateSmoke();
        process.exitCode = 0;
    } catch (error) {
        logger.error("Grpc streaming realtime main server two client player state smoke failed", {
            target: DEFAULT_TARGET,
...
//* توضیح کلی فایل:
//* این فایل تست مستقل دو کلاینت جی‌آر‌پی‌سی و وضعیت پلیر روی سرور اصلی در حال اجرا است.
...
//* سپس کاربر دوم پیام وضعیت پلیر می‌فرستد.
//* دریافت همان وضعیت پلیر روی اِستریم کاربر اول و دریافت اَک روی اِستریم کاربر دوم کنترل می‌شود.
//* در پایان داده‌های تستی از دیتابیس پاک می‌شوند.
//* هدف این فایل اثبات سینک وضعیت پلیر بین دو کلاینت جی‌آر‌پی‌سی روی اجرای اصلی سرور است.

export {
    runGrpcStreamingRealtimeMainServerTwoClientPlayerStateSmoke,
    ...
    assertPlayerStateFrame,
    assertPlayerStateAckFrame
};
