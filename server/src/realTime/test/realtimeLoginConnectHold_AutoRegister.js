// File => src/realTime/test/realtimeLoginConnectHold_AutoRegister.js
...
const AutoRegister = ["1", "true", "yes", "on"].includes(String(process.env.REALTIME_TEST_AUTO_REGISTER || "").toLowerCase());
const Email = process.env.REALTIME_TEST_EMAIL || process.env.TEST_EMAIL || (AutoRegister ? `realtime-hold-${Date.now()}@example.com` : "");
const Password = process.env.REALTIME_TEST_PASSWORD || process.env.TEST_PASSWORD || (AutoRegister ? "12345678" : "");
const UserName = process.env.REALTIME_TEST_USERNAME || "";
const DirectAccessToken = process.env.REALTIME_TEST_ACCESS_TOKEN || "";
...
function makeTestMetadata() {
    ...
    metadata.set("user-agent", "realtime-login-connect-hold-node");
    return metadata;
}

function isInvalidCredentialsError(error) {
    const message = String(error?.details || error?.message || "").toLowerCase();
    return error?.code === grpc.status.UNAUTHENTICATED || message.includes("invalid credentials");
}

async function registerAndGetAccessToken(client) {
    if (!Email || !Password) fail("REALTIME_TEST_EMAIL and REALTIME_TEST_PASSWORD are required for register");

    log("gRPC register started", { target: GrpcTarget, email: Email, hasUserName: Boolean(UserName) });
    const response = await callUnary(client, "Register", { email: Email, password: Password, userName: UserName }, makeTestMetadata());

    if (!response?.success || !response?.accessToken) fail("gRPC register failed", { success: response?.success, message: response?.message });
    log("gRPC register passed", { userId: response?.user?.id ?? "", email: Email, expiresIn: response?.expiresIn ?? 0 });
    return response.accessToken;
}

async function loginAndGetAccessToken() {
    if (DirectAccessToken) {
        log("direct access token mode enabled", { tokenLength: DirectAccessToken.length });
        return DirectAccessToken;
    }

    if (!Email || !Password) fail("REALTIME_TEST_EMAIL and REALTIME_TEST_PASSWORD are required unless REALTIME_TEST_ACCESS_TOKEN is set");

    const client = loadAuthClient();

    try {
        log("gRPC login started", { target: GrpcTarget, email: Email, hasUserName: Boolean(UserName), autoRegister: AutoRegister });
        const response = await callUnary(client, "Login", { email: Email, password: Password, userName: UserName }, makeTestMetadata());
        ...
        if (!AutoRegister || !isInvalidCredentialsError(error)) throw error;
        log("gRPC login failed; auto register is enabled", { code: error?.code, details: error?.details || error?.message || "" });
        return await registerAndGetAccessToken(client);
    } finally {
        client.close?.();
    }
}
...
async function main() {
    log("test_started", { grpcTarget: GrpcTarget, wsUrl: WsUrl, holdMs: HoldMs, runId: TestRunId, autoRegister: AutoRegister, email: Email });
